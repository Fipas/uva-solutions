...
int main() {
	...
	for (int c = 0; c < t; c++) {
		...
		int field, numBlue, numGreen, power;
		multiset<int, greater<int> > armyBlue, armyGreen;

		cin >> field >> numGreen >> numBlue;

		for (int i = 0; i < numGreen; i++) {
			cin >> power;
			armyGreen.insert(power);
		}

		for (int i = 0; i < numBlue; i++) {
			cin >> power;
			armyBlue.insert(power);
		}		

		while (armyBlue.size() && armyGreen.size()) {
			multiset<int, greater<int> >::iterator itBlue, itGreen;
			queue<int> toInsertGreen, toInsertBlue;
			int i;

			for (i = 0, itBlue = armyBlue.begin(), itGreen = armyGreen.begin(); 
				 i < field && itBlue != armyBlue.end() && itGreen != armyGreen.end(); 
				 i++) {
				int auxGreen = *(itGreen) - *(itBlue);
				int auxBlue = *(itBlue) - *(itGreen);

				//cout << auxGreen << "	" << auxBlue << endl;

				multiset<int, greater<int> >::iterator itBlueNext, itGreenNext;
				...
				armyGreen.erase(itGreen);
				armyBlue.erase(itBlue);

				if (auxGreen > 0)
					toInsertGreen.push(auxGreen);

				if (auxBlue > 0)
					toInsertBlue.push(auxBlue);

				itGreen = itGreenNext;
				itBlue = itBlueNext;
			}

			while (!toInsertGreen.empty()) {
				armyGreen.insert(toInsertGreen.front());
				toInsertGreen.pop();
			}

			while (!toInsertBlue.empty()) {
				armyBlue.insert(toInsertBlue.front());
				toInsertBlue.pop();
			}
		}

		if (armyBlue.size() > armyGreen.size()) {
			cout << "blue wins\n";
			multiset<int>::iterator it;
			for (it = armyBlue.begin(); it != armyBlue.end(); ++it)
				cout << *it << endl;
		} else if (armyGreen.size() > armyBlue.size()) {
			cout << "green wins\n";
			multiset<int>::iterator it;
			for (it = armyGreen.begin(); it != armyGreen.end(); ++it)
				cout << *it << endl;
		} else {
