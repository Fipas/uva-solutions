...
bool allClear(vector<queue<int> > stations) {
	bool clear = true;

	for (int i = 0; i < stations.size(); i++)
		clear = clear && (stations[i].empty());
...
int main() {
	...
	while (t--) {
		...
		while (true) {
			...
			while (!carrier.empty() && (carrier.top() == sp || stations[sp].size() < qs)) {
				if (carrier.top() != sp)
					stations[sp].push(carrier.top());
				carrier.pop();
				c++;
			}

			while (carrier.size() < ss && !stations[sp].empty()) {
				carrier.push(stations[sp].front());
				stations[sp].pop();
				c++;
			}
			...
			if (carrier.empty() && allClear(stations)) {
				break;
			}
