...
int main() {
	...
	while (cin >> n && n) {
		...
		int maxPopularity = -1, numStudents = 0;

		for (int i = 0; i < n; i++) {
			int v;
			for (int j = 0; j < 5; j++) {
				cin >> v;
				students[i].push_back(v);
			}
			sort(students[i].begin(), students[i].end());
			popularity[students[i]]++;
			maxPopularity = max(maxPopularity, popularity[students[i]]);
		}

		for (int i = 0; i < n; i++) {
			if (popularity[students[i]] == maxPopularity)
				numStudents++;
		}
