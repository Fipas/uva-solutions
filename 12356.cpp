...
int main() {
	...
	while (scanf("%d %d", &soldiers, &losses) && soldiers && losses) {
		vector<pair<int, int> > alive(soldiers + 1);

		alive[0] = make_pair(-1, -1);

		for (int i = 1; i <= soldiers; i++) {
			alive[i] = make_pair(i - 1, i + 1);
		}

		alive[1].first = -1;
		alive[soldiers].second = -1;
		...
		while (losses--) {
			...
			if (alive[l].first != -1) {
				printf("%d ", alive[l].first);
				alive[alive[l].first].second = alive[r].second;
			} else {
				printf("* ");
			}
			...
			if (alive[r].second != -1) {
				printf("%d\n", alive[r].second);
				alive[alive[r].second].first = alive[l].first;
			} else {
				printf("*\n");
			}
		}
