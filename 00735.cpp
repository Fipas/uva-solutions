...
int main() {
	...
	while (cin >> n && n > 0) {
		...
		for (int i = 0; i < vs.size(); ++i) {
			for (int j = 0; j < vs.size(); ++j) {
				for (int k = 0; k < vs.size(); ++k) {
					...
					int s1 = vs[i] + vs[j] + vs[k];

					if (n == s1) {
						perm++;

						if (!visited[vs[i]][vs[j]][vs[k]]) {
							com++;
							visited[vs[i]][vs[j]][vs[k]] = true;
							visited[vs[i]][vs[k]][vs[j]] = true;
							visited[vs[j]][vs[k]][vs[i]] = true;
							visited[vs[j]][vs[i]][vs[k]] = true;
							visited[vs[k]][vs[i]][vs[j]] = true;
							visited[vs[k]][vs[j]][vs[i]] = true;
						}
					}
				}
			}
		...
		if (com != 0 || perm != 0) {
			cout << "NUMBER OF COMBINATIONS THAT SCORES " << n << " IS " << com << ".\n";
