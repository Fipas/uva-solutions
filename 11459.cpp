...
int main() {
	...
	for (int i = 0; i < t; i++) {
		...
		map<int, bool> isSnake;
		map<int, int> snakeTailPos;

		for (int j = 0; j < snakes; j++) {
			int smouth, stail;
			cin >> smouth >> stail;
			isSnake[smouth] = true;
			snakeTailPos[smouth] = stail;
		}
		...
		for (int j = 0; j < rolls; j++) {
			...
			pos[j % players] += roll;
			if (pos[j % players] >= 100) {
				gameover = true;
				pos[j % players] = 100;
			} else if (isSnake[pos[j % players]]) {
				pos[j % players] = snakeTailPos[pos[j % players]];
				if (pos[j % players] >= 100) {
					gameover = true;
					pos[j % players] = 100;
				}
		...
		for (int j = 0; j < players; j++) {
			cout << "Position of player " << j + 1 << " is " << pos[j] << ".\n";
		}
	}
