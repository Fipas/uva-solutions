...
int main() {
	...
	while (cin >> n) {
		...
		for (int i = 0; i < m; i++) {
			...
			//cout << dict[line[0]] << "		" << dict[line[1]] << endl;
			cons.PB(MP(dict[line[0]], dict[line[1]]));
		}

		oldDisjoint = UF.numDisjointSets() + 1;

		//cout << UF.numDisjointSets() << endl;

		while (UF.numDisjointSets() != 1 && oldDisjoint != UF.numDisjointSets()) {
			years++;
			oldDisjoint = UF.numDisjointSets();
			vi conNum(n + 1, 0);
