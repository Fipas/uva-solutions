...
int main() {
	...
	while (cin >> n >> m) {
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = 0; j < nm; j++) {
				cin >> v;
				adjList[i].PB(make_pair(v - 1, 0));
			}


			for (int j = 0; j < nm; j++) {
				cin >> v;
				adjList[i][j].second = v;
		...
		for (int i = 0; i < adjList.size(); i++) {
			for (int j = 0; j < adjList[i].size(); j++) {
				transAdjList[adjList[i][j].first].PB(make_pair(i, adjList[i][j].second));
			}
		}
