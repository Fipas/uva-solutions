	...
	for (int c = 0; c < t; c++) {
		...
		while (queries--) {
			...
			for (int i = 0; i < maker_list.size(); i++) {
				if ((price >= maker_list[i].lowest && price <= maker_list[i].highest) ||
					(price > maker_list[i].lowest && price <= maker_list[i].highest) || 
					(price >= maker_list[i].lowest && price < maker_list[i].highest)) {
					if (found) {
						found = false;
						break;
			...
			if (found) {
				cout << maker_list[hits].name << endl;
			} else {
				cout << "UNDETERMINED\n";
			}
